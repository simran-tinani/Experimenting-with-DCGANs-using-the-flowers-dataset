...
class DCGAN(object):
    ...
    def generator(self):
        ...
        self.G.add(BatchNormalization(momentum=0.9)) # Batch Normalization
...
class flowers_DCGAN(object):
    def __init__(self): # initializing flowers_DCGAN
        self.img_rows = 28
        self.img_cols = 28
        self.channel = 3
        self.x_train = flowers
        self.x_train = self.x_train.reshape(-1, self.img_rows,\
        	self.img_cols, 3).astype(np.float32) # reshape 784*1 to 28*28*1
    ...
    def train(self, train_steps=2000, batch_size=56, save_interval=0): # batch size for minibatch GD
        noise_input = None
        if save_interval>0:
...
# generates output images using the noise
    ...
    def plot_images(self, save2file=False, fake=True, samples=16, noise=None, step=0):
        filename1 = 'flowers.png'
        if fake:
            if noise is None:
                noise = np.random.uniform(-1.0, 1.0, size=[samples, 100])
            else:
                filename1 = "flowers1_%d.png" % step
                # filename2 = "flowers2_%d.png" % step
...
if __name__ == '__main__':
    ...
    timer = ElapsedTimer()
    flowers_dcgan.train(train_steps=10000, batch_size=56, save_interval=1000)
    timer.elapsed_time()
